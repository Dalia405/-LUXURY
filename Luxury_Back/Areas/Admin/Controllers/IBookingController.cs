...
namespace Luxury_Back.Controllers.Admin
{
    ...
    public class IBookingController : Controller
    {
        ...
        {
             bool isFound = luxuryDb.iBookings.Include(i => i.Address).Include(x => x.iBookingAttributes).Any(att => att.Id == iBooking.Id);
            if (!isFound)
            {
            ...
            using (IDbContextTransaction transaction = luxuryDb.Database.BeginTransaction())
            {
                try
                {
                    luxuryDb.Entry(iBooking).State = EntityState.Modified;
